...
class ObjectDict(dict):
        ...
        if not objpath:
            ...
            try:
                ...
                if isinstance(item, list):
                    node = int(node, 0)
                yield from ObjectDict._query(
                    item[node], objpath[1:], qpath + [str(node)], qparam
                )
    ...
    def select_one(self, objpath, copy=False, **kwargs):
        ...
        if len(result) != 1:
            raise Exception(
                "Expected exactly 1 of {}, found {}".format(objpath, len(result))
            )
        return result[0]
...
def Tile(bank, char):
    if isinstance(bank, Address.Chr):
        return Address.Chr(bank.bank(), char * 16)
    elif isinstance(bank, Address.Chr1k):
        return Address.Chr1k(bank.bank(), char * 16)
    else:
        return Address.Chr(bank, char * 16)


def Tile1k(bank, char):
    if isinstance(bank, Address.Chr1k):
        bank = bank.bank()
    return Address.Chr1k(bank, char * 16)
...
def chr_copy(rom, dst_tile, src_tile):
    if not isinstance(dst_tile, (Address.Chr, Address.Chr1k)):
        raise Exception('dst_tile address not in "chr" segment')
    if not isinstance(src_tile, (Address.Chr, Address.Chr1k)):
...
def chr_swap(rom, a_tile, b_tile):
    if not isinstance(a_tile, (Address.Chr, Address.Chr1k)):
        raise Exception('a_tile address not in "chr" segment')
    if not isinstance(b_tile, (Address.Chr, Address.Chr1k)):
