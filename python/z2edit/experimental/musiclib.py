...
class Pattern(object):
    ...
    @staticmethod
    def from_rom(rom, address):
        tempo = rom.read(address + 0)
        data = rom.read_pointer(address + 1)
        tri = rom.read(address + 3)
        pw2 = rom.read(address + 4)
        noi = rom.read(address + 5)

        ret = Pattern(tempo)
        ret.read_notes(rom, Channel.Pulse1, data)
        if tri: ret.read_notes(rom, Channel.Triangle, data+tri)
        if pw2: ret.read_notes(rom, Channel.Pulse2, data+pw2)
        if noi: ret.read_notes(rom, Channel.Noise, data+noi)
        return ret
...
class Song(object):
    @staticmethod
    def from_rom(rom, address, entry):
        table = rom.read_bytes(address, 8)
        offsets = {}
        sequence = []
        pattern = []
        i = 0
        n = 0
        while True:
            offset = rom.read(address + table[entry] + i)
            if offset == 0:
                break
            if offset not in offsets:
                offsets[offset] = n
                n += 1
                pattern.append(Pattern.from_rom(rom, address+offset))
    ...
    @staticmethod
    def read_all_songs(rom):
        return {
            "OverworldIntro":  Song.from_rom(rom, OVERWORLD_SONG_TABLE, 0),
            "OverworldTheme":  Song.from_rom(rom, OVERWORLD_SONG_TABLE, 1),
            "BattleTheme":     Song.from_rom(rom, OVERWORLD_SONG_TABLE, 2),
            "CaveItemFanfare": Song.from_rom(rom, OVERWORLD_SONG_TABLE, 4),

            "TownIntro":       Song.from_rom(rom, TOWN_SONG_TABLE, 0),
            "TownTheme":       Song.from_rom(rom, TOWN_SONG_TABLE, 1),
            "HouseTheme":      Song.from_rom(rom, TOWN_SONG_TABLE, 2),
            "TownItemFanfare": Song.from_rom(rom, TOWN_SONG_TABLE, 4),

            "PalaceIntro":       Song.from_rom(rom, PALACE_SONG_TABLE, 0),
            "PalaceTheme":       Song.from_rom(rom, PALACE_SONG_TABLE, 1),
            "BossTheme":         Song.from_rom(rom, PALACE_SONG_TABLE, 3),
            "PalaceItemFanfare": Song.from_rom(rom, PALACE_SONG_TABLE, 4),
            "CrystalFanfare":    Song.from_rom(rom, PALACE_SONG_TABLE, 6),

            "GreatPalaceIntro":       Song.from_rom(rom, GP_SONG_TABLE, 0),
            "GreatPalaceTheme":       Song.from_rom(rom, GP_SONG_TABLE, 1),
            "ZeldaTheme":             Song.from_rom(rom, GP_SONG_TABLE, 2),
            "CreditsTheme":           Song.from_rom(rom, GP_SONG_TABLE, 3),
            "GreatPalaceItemFanfare": Song.from_rom(rom, GP_SONG_TABLE, 4),
            "TriforceFanfare":        Song.from_rom(rom, GP_SONG_TABLE, 5),
            "FinalBossTheme":         Song.from_rom(rom, GP_SONG_TABLE, 6),
        }

    @staticmethod
    def convert_all_to_famistudio(rom, filename='songs-fms.txt'):
        song = Song.read_all_songs(rom)
...
""", file=f)
    ...
    def commit(rom, address, songs):
        ...
        for i in range(8):
            rom.write(address+i, offsets[table[i]])
        ...
        for s in songs:
            ...
            rom.write_bytes(address + seq_offset, data)
            pat_offset += 6 * len(s.pattern)
            seq_offset += len(data)
        rom.write(address+seq_offset, 0)
        ...
        for s in songs:
            for p in s.pattern:
                ...
                rom.write_bytes(address+pat_offset, p.meta_data(note_address))
                rom.write_bytes(note_address, data)
                pat_offset += 6
