...
def palace2_must_be_connection52(rom):
    """Palace 2 must be connection #52"""
    logger.info("Applying palace2_must_be_connection52")
    asm = Asm(rom)
    asm("""
        .bank 4
        .org $bac3
...
def palace2_even_palace5_odd(rom):
    """P2 even / P5 odd"""
    logger.info("Applying palace2_even_palace5_odd")
    length = 7
    freespace = rom.alloc(Address.Prg(4, 0xbfe0), length)
...
def palace2_odd_palace5_even(rom):
    """P2 odd / P5 even"""
    logger.info("Applying palace2_odd_palace5_even")
    length = 7
    freespace = rom.alloc(Address.Prg(4, 0xbfe0), length)
    logger.info("Using freespace at %s", freespace)
    freespace = freespace.offset()
...
def continue_from_palaces(rom):
    """Continue from any Palace"""
    logger.info("Applying continue_from_palaces")
    length = 45
    freespace = rom.alloc(Address.Prg(-1, 0xd39a), length)
    logger.info("Using freespace at %s", freespace)
    freespace = freespace.offset()
    asm = Asm(rom)
    asm(f"""
...
def palaces_to_stone_fixed(rom):
    ...
    """
    logger.info("Applying palaces_to_stone_fixed")
    asm = Asm(rom)
    asm("""
        .bank 1
        .org  $87aa
...
def palaces_to_stone_never(rom):
    """Completely disable palaces turning to stone."""
    logger.info("Applying palaces_to_stone_never")
    asm = Asm(rom)
    asm("""
        .bank -1
        .org  $e01e
