...
class Asm:
    ...
    def parse_line(self, line):
        ...
        if mode is None:
            raise OperandError(
                "Invalid Operand", opcode, operand, self.linenum, orig_line
            )
        if info[mode] == -1:
            if info[Asm.RELATIVE] != -1:
                mode = Asm.RELATIVE
            if info[Asm.IMPLIED] != -1:
                mode = Asm.IMPLIED
            if info[Asm.FAKE] != -1:
                mode = Asm.FAKE
        if info[mode] == -1:
            raise OperandError(
                "Invalid addressing mode", opcode, operand, self.linenum, orig_line
            )
        ...
        if (
            ...
            if fixup_resolved:
                ...
                if delta < -128 or delta > 127:
                    raise OperandError(
                        "Bad displacement", self.org, delta, self.linenum, orig_line
                    )
            else:
                delta = 0xFF
            if not fixup_resolved:
            ...
            self.org += 2
        elif mode == Asm.ACCUMULATOR or mode == Asm.IMPLIED:
            self.write(self.org, info[mode])
            self.org += 1
        elif mode == Asm.FAKE:
            if not fixup_resolved:
            ...
            if opcode == ".ORG":
                ...
                if self.org != addr:
                    raise OperandError(
                        "ORG assertion failed on line %d (org=$%04x, expected=$%04x, delta since last org=%d)"
                        % (self.linenum, self.org, addr, self.org - self.last_org),
                        self.linenum,
                        self.org,
                        addr,
                        self.org - self.last_org,
                        orig_line,
                    )
            elif opcode == ".BANK":
                self.bank = addr
