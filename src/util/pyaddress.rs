...
impl PyAddress {
    ...
    fn file(addr: usize) -> PyAddress {
        PyAddress { address: Address::File(addr) }
    }

    #[staticmethod]
    fn segment(segment: &str, addr: usize) -> PyAddress {
        PyAddress { address: Address::new_seg(segment, addr) }
    }

    #[staticmethod]
    fn banked(bankname: &str, bank: isize, addr: usize) -> PyAddress {
        PyAddress { address: Address::new_bank(bankname, bank, addr) }
    }

    #[staticmethod]
    fn prg(bank: isize, addr: u16) -> PyAddress {
        PyAddress { address: Address::Prg(bank, addr) }
    }

    #[staticmethod]
    fn chr(bank: isize, addr: u16) -> PyAddress {
        PyAddress { address: Address::Chr(bank, addr) }
    }
