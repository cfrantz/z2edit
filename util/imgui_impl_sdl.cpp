...
static float        hidpi_scale = 1.0f;

void ImGui_ImplSdl_SetHiDPIScale(float scale) {
    hidpi_scale = scale;
}

float ImGui_ImplSdl_GetHiDPIScale() {
    return hidpi_scale;
}
...
void ImGui_ImplSdl_NewFrame(SDL_Window *window)
{
    ...
    if (SDL_GetWindowFlags(window) & SDL_WINDOW_MOUSE_FOCUS)
        io.MousePos = ImVec2((float)mx/scale, (float)my/scale);   // Mouse position, in pixels (set to -1,-1 if no mouse / on another screen, etc.)
    else
