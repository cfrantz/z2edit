...
impl CNROM {
    pub fn new(rom: &NesFile) -> Result<Self> {
        let banks = u8::try_from(rom.chr_banks())?;
        Ok(Self {
            chr_banks: banks,
            chr_bank1: 0,
    ...
    fn read(&mut self, nes: &Nes, address: Address) -> u8 {
        ...
        match address {
            Address::Cpu(addr) => match addr {
                ...
                0x8000..=0xFFFF => {
                    // Address::Prg conceptualizes 16k bank, but CNROM has only a
                    // single 32k bank. Calculate which 16k bank we're addressing.
                    let bank = (addr & 0x7FFF) / 0x4000;
                    let a = Address::Prg(bank as i16, addr);
                    match rom.read(a) {
                        Ok(v) => v,
                        Err(e) => {
                            log::error!("CNROM failed to read rom {a:x?}: {e}");
                            0xff
                        }
                    }
                }
                _ => {
                    log::warn!("CNROM: unhandled CPU read: address={:04x}", addr);
                    0
                }
            },
            Address::Ppu(addr) => match addr {
                0x0000..=0x1FFF => {
                    // Address::Chr conceptualizes 4k bank, but CNROM maps a
                    // single 8k CHR mapping. Calculate which bank we're addressing.
                    let bank = (addr / 0x1000) as i16;
                    let a = Address::Chr(self.chr_bank1 as i16 * 2 + bank, addr);
                    match rom.read(a) {
                        Ok(v) => v,
                        Err(e) => {
                            log::error!("CNROM failed to read rom {a:x?}: {e}");
                ...
                _ => {
                    log::warn!("CNROM: unhandled PPU read: address={:04x}", addr);
                    0
                }
            },
            _ => {
                log::warn!("CNROM: unhandled read for address space: {:?}", address);
                0
            }
        }
    }
    ...
    fn write(&mut self, nes: &Nes, address: Address, val: u8) {
        match address {
            Address::Cpu(addr) => match addr {
                ...
                0x8000..=0xFFFF => {
                    self.chr_bank1 = val & 3;
                }
                _ => {
                    log::warn!(
                        "CNROM: unhandled CPU write address={:04x} value={:02x}",
                        addr,
                        val
                    );
                }
            ...
            Address::Ppu(addr) => match addr {
                0x0000..=0x1FFF => {
                    ...
                    // For CNROM with CHR RAM, this would write to CHR RAM.
                    // Address::Chr conceptualizes 4k bank, but CNROM maps a
                    // single 8k CHR mapping. Calculate which bank we're addressing.
                    let bank = (addr / 0x1000) as i16;
                    let a = Address::Chr(self.chr_bank1 as i16 * 2 + bank, addr);
                ...
                _ => {
                    log::warn!(
                        "CNROM: unhandled PPU write address={:04x} value={:02x}",
                        addr,
                        val
                    );
                }
            ...
            _ => {
                log::warn!(
                    "CNROM: unhandled write for address space: {:?} value {:02x}",
                    address,
                    val
                );
            }
